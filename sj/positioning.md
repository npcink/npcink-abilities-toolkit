...
Npcink Abilities Toolkit can participate in a Npcink AI host setup:

- `npcink-abilities-toolkit` - ability definitions and ability callbacks.
- `npcink-ai-core` - governance, approval, preflight, and audit.
- `npcink-ai-adapter` - OpenClaw channel adaptation that calls Core and the
  Abilities API.
- `npcink-ai-cloud-addon` - cloud service connection.
...
Npcink Abilities Toolkit 是 Npcink AI 系列插件的一部分：

- `npcink-abilities-toolkit` - 能力定义和 ability callback。
- `npcink-ai-core` - 治理、审批、preflight、audit。
- `npcink-ai-adapter` - OpenClaw 通道适配，调用 Core 和 Abilities API。
- `npcink-ai-cloud-addon` - 链接云端服务。
