...
trait Post_Primitives_Read_Methods {
	...
	public function search_posts( $input ) {
		...
		if ( '' === $search ) {
			return new \WP_Error( 'npcink_abilities_toolkit_search_empty', __( 'Search keyword cannot be empty.', 'npcink-abilities-toolkit' ), array( 'status' => 400 ) );
		}
	...
	public function list_revisions( $input ) {
		...
		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			return new \WP_Error( 'npcink_abilities_toolkit_permission_denied', __( 'You do not have permission to read this post revision history.', 'npcink-abilities-toolkit' ), array( 'status' => 403 ) );
		}
	...
	public function search_post_meta( $input ) {
		...
		if ( empty( $meta_keys ) ) {
			return new \WP_Error( 'npcink_abilities_toolkit_meta_keys_required', __( 'At least one non-sensitive meta key is required.', 'npcink-abilities-toolkit' ), array( 'status' => 400 ) );
		}
		if ( empty( $post_type_filter ) ) {
			return new \WP_Error( 'npcink_abilities_toolkit_post_type_invalid', __( 'Post type does not exist.', 'npcink-abilities-toolkit' ), array( 'status' => 400 ) );
		}
	...
	public function resolve_url_to_post( $input ) {
		...
		if ( '' === $url && '' === $slug ) {
			return new \WP_Error( 'npcink_abilities_toolkit_resolve_input_required', __( 'A URL or slug is required.', 'npcink-abilities-toolkit' ), array( 'status' => 400 ) );
		}
		...
		if ( $post_id <= 0 ) {
			return new \WP_Error( 'npcink_abilities_toolkit_post_not_found', __( 'No matching post was found.', 'npcink-abilities-toolkit' ), array( 'status' => 404 ) );
		}
	...
	public function get_post_blocks( $input ) {
		...
		if ( $revision_id > 0 ) {
			...
			if ( ! $revision || 'revision' !== sanitize_key( (string) ( $revision->post_type ?? '' ) ) ) {
				return new \WP_Error( 'npcink_abilities_toolkit_revision_not_found', __( 'Revision was not found.', 'npcink-abilities-toolkit' ), array( 'status' => 404 ) );
			}
			if ( absint( $revision->post_parent ?? 0 ) !== $post_id ) {
				return new \WP_Error( 'npcink_abilities_toolkit_revision_post_mismatch', __( 'Revision does not belong to the requested post.', 'npcink-abilities-toolkit' ), array( 'status' => 400 ) );
			}
