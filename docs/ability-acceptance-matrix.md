...
| Media governance | `magick-ai/get-media-cleanup-opportunities`, `magick-ai/build-media-inventory-fix-plan`, `magick-ai/build-media-seo-assets`, `magick-ai/optimize-media-metadata`, `magick-ai/inspect-media-asset`, `magick-ai/update-media-details`, `magick-ai/optimize-media-asset`, `magick-ai/replace-media-file`, `magick-ai/delete-media-permanently` | Agent can separate read-only cleanup/SEO/format recommendations, reviewable dry-run media remediation plans, and host-approved metadata updates, derivative generation, recorded-derivative replacement with backup/rollback metadata, or destructive deletes. | `composer test`, `composer smoke:wp` single-ability runs for read, planning, and governed dry-run context. | Read and planning side accepted; write/destructive side remains host approval only. |
