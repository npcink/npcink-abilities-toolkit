...
| Media governance | `magick-ai/get-media-cleanup-opportunities`, `magick-ai/build-media-inventory-fix-plan`, `magick-ai/build-media-seo-assets`, `magick-ai/optimize-media-metadata`, `magick-ai/inspect-media-asset`, `magick-ai/build-media-derivative-cloud-request`, `magick-ai/build-media-derivative-batch-plan`, `magick-ai/update-media-details`, `magick-ai/optimize-media-asset`, `magick-ai/replace-media-file`, `magick-ai/delete-media-permanently` | Agent can separate read-only cleanup/SEO/format recommendations, reviewable dry-run media remediation plans, Cloud derivative request contracts, bounded batch derivative candidate plans, and host-approved metadata updates, derivative generation, recorded-derivative replacement with backup/rollback metadata, or destructive deletes. | `composer test`, `composer smoke:wp` single-ability runs for read, planning, and governed dry-run context. | Read and planning side accepted; write/destructive side remains host approval only. |
