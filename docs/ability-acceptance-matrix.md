...
| Media governance | `magick-ai/get-media-cleanup-opportunities`, `magick-ai/build-media-inventory-fix-plan`, `magick-ai/build-media-seo-assets`, `magick-ai/optimize-media-metadata`, `magick-ai/update-media-details`, `magick-ai/delete-media-permanently` | Agent can separate read-only cleanup/SEO recommendations, reviewable dry-run media remediation plans, and host-approved metadata updates/destructive deletes. | `composer test`, `composer smoke:wp` single-ability runs for read and planning context. | Read and planning side accepted; write/destructive side remains host approval only. |
